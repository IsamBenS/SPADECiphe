...
SPADE.addDensityToFCSCIPHE <- function(
	...
	if (max(density) == 0.0)
		...
		warning(paste("fcs file has degenerate densities, possibly due to many identical observations",sep=" "))
	...
	keyval[[paste("flowCore_$P",channel_number,"Rmax",sep="")]] <- toString(channel_range-1);		# ADDED 11/10/2017
	keyval[[paste("flowCore_$P",channel_number,"Rmin",sep="")]] <- toString(0);		# ADDED 11/10/2017
...
SPADE.downsampleFCSCIPHE <- function(
	...
	if (!is.null(target_percent)) {
		...
		message("Targeting ", target_number, " events for fcs file")
